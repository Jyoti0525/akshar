...
- [x] B3 transform matrix **retained in the context** so every OCR box maps back onto the original photograph for the annotated evidence image — built 2026-09-18. `vision/types.py::Transform` carries the forward homography, both frame sizes and the inverse; `ScanOutcome.transform` and `ScanContext.transform` are the B3 slot. **A box does not map back to a box** and the type refuses to pretend otherwise: a perspective warp does not preserve right angles, so `box_to_original` returns a `Quad` and there is deliberately no method here returning an `XYWH`. Drawing the bounding box of those four corners would mark a region *larger than the one measured*, on an exhibit whose entire purpose is to show exactly what was measured — a test asserts the bounding box is strictly larger than the quad. Corner order is preserved so a drawn polygon never self-intersects; a singular matrix returns `None` rather than raising, because a degenerate homography is a bad photograph and the cost is one missing annotation, never a failed scan

- [~] **B7 two-pass recognition** — **built, guarded, tested, and switched OFF on the evidence.** `vision/ocr/second_pass.py` maps a doubtful line's box back through B3's transform, warps it out of the *original* photograph at the quad's native density, reduces to the head's 48 px input with `INTER_AREA`, and re-reads. One properly-filtered resampling in place of three unfiltered ones. It replaces text, confidence and engine and **never geometry** — a cap height measured on the sharper crop would be more precise and in a different pixel scale, and `mm_per_px` is calibrated against rectified space
  - **The defect the corpus caught, which would have shipped silently.** The first version accepted a second reading whenever it was more confident. `ctc_decode` sets confidence to the **mean** of the kept per-step scores, so a decoder that drops the characters it finds hardest returns a *higher* score for a worse reading. Measured on 20 frames: **17 text changes, of which one was right** — `' c.N 1310010409'` became `'  No 130029'`, four digits of a licence number gone, recorded as an improvement. The rule is now: **no shorter than the first reading** (characters dropped are evidence dropped) **and** a gain of at least `MIN_CONFIDENCE_GAIN`. Same 20 frames after the fix: 2 text changes, one of them a clear fix. Two tests pin both halves
  - **Then measured properly, and it does not pay for itself yet.** The bench was run twice back to back over the 38 hand-labelled panels — the only ground truth this project has. Presence micro F1 **0.7506 both times**; every per-field P/R identical; every value-accuracy count identical (mrp 19/38, net_quantity 25/34, mfg_date 14/32). The two bench JSONs differ **only** in their timing fields. Against that: timed in isolation it is **median 0.2 ms** (most scans have nothing below threshold and it never starts) but fires on **4 of 14** frames, and when it fires costs **336 ms mean, up to 953 ms**, against §4's 561 ms budget for the whole scan
  - On 70 unlabelled corpus frames / 1,915 lines it changed 11 texts. Some are plainly right (`'[014814'`->`'1014814'`, `'6'`->`'760'`, `'A='`->`'59.82'`), at least one plainly wrong (`'THORN'`->`'THDEA'`), and the rest cannot be scored **because the corpus has no ground truth**. `ENABLED = False` carries that measurement in full and the pipeline reads it, so turning it on is one boolean rather than a re-integration. The set that would settle it is the one already asked for: the 38 panels are close-ups whose rectification barely resamples, which is exactly where this pass has least to offer
